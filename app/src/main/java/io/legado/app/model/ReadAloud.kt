...
object ReadAloud {
    ...
    private fun getReadAloudClass(): Class<*> {
        ...
        if (ttsEngine == EdgeTts.ENGINE_ID) {
            //Edge TTS 内置引擎, 以伪HttpTTS承载, url携带声音与朗读模式用于缓存区分
            httpTTS = HttpTTS(
                name = "Edge TTS",
                url = "edge-tts://${AppConfig.edgeTtsVoice}|${AppConfig.edgeTtsVoiceEn}|${AppConfig.edgeTtsReadMode}"
            )
            return HttpReadAloudService::class.java
        }
        if (ttsEngine == OpenAiCompatTts.DASHSCOPE_ENGINE_ID) {
            //Qwen3 TTS (阿里 DashScope), url携带模型与音色用于缓存区分
            httpTTS = HttpTTS(
                name = "Qwen3 TTS",
                url = "dashscope-tts://${AppConfig.dashscopeModel}|${AppConfig.dashscopeVoice}"
            )
            return HttpReadAloudService::class.java
        }
        if (ttsEngine == OpenAiCompatTts.OPENAI_ENGINE_ID) {
            //OpenAI 兼容 TTS (VibeVoice/Confucius等自建), url携带模型与音色用于缓存区分
            httpTTS = HttpTTS(
                name = "OpenAI TTS",
                url = "openai-tts://${AppConfig.openaiTtsModel}|${AppConfig.openaiTtsVoice}"
            )
            return HttpReadAloudService::class.java
        }
