...
object AppConfig : SharedPreferences.OnSharedPreferenceChangeListener {
    ...
    //墨水屏界面风格(独立开关, 不随关灯/开灯丢失): 纯白界面与对话框/黑边弹窗/无动画
    var isEInkUiStyle: Boolean
        get() = appCtx.getPrefBoolean(PreferKey.isEInkUiStyle, true)
        set(value) {
            appCtx.putPrefBoolean(PreferKey.isEInkUiStyle, value)
        }

    //夜间模式下让位给暗色主题, 避免关灯时界面/状态栏仍是白底
    val isEInkMode: Boolean
        get() = isEInkUiStyle && !isNightTheme
    ...
    var themeMode = appCtx.getPrefString(PreferKey.themeMode, "0")
        set(value) {
            //旧配置迁移: 墨水屏模式("3")已移除, 回退跟随系统
            if (value == "3") {
                field = "0"
                appCtx.putPrefString(PreferKey.themeMode, "0")
            } else {
                field = value
            }
        }
    ...
    var edgeTtsVoice: String
        get() = appCtx.getPrefString(PreferKey.edgeTtsVoice) ?: EdgeTts.DEFAULT_VOICE
        set(value) {
            appCtx.putPrefString(PreferKey.edgeTtsVoice, value)
        }

    //Edge TTS 英文声音(未手动设置时按中文声音性别自动匹配)
    var edgeTtsVoiceEn: String
        get() = appCtx.getPrefString(PreferKey.edgeTtsVoiceEn)
            ?: EdgeTts.defaultEnVoice(edgeTtsVoice)
        set(value) {
            appCtx.putPrefString(PreferKey.edgeTtsVoiceEn, value)
        }

    //Edge TTS 朗读模式: 0中文音色读中英文 1中英文分别用各自音色 2英文音色读中英文
    var edgeTtsReadMode: Int
        get() = appCtx.getPrefInt(PreferKey.edgeTtsReadMode, 1)
        set(value) {
            appCtx.putPrefInt(PreferKey.edgeTtsReadMode, value)
        }

    //通义千问 TTS (DashScope)
    var dashscopeApiKey: String
        get() = appCtx.getPrefString(PreferKey.dashscopeApiKey) ?: ""
        set(value) {
            appCtx.putPrefString(PreferKey.dashscopeApiKey, value)
        }

    var dashscopeModel: String
        get() = appCtx.getPrefString(PreferKey.dashscopeModel)
            ?: OpenAiCompatTts.DASHSCOPE_DEFAULT_MODEL
        set(value) {
            appCtx.putPrefString(PreferKey.dashscopeModel, value)
        }

    var dashscopeVoice: String
        get() = appCtx.getPrefString(PreferKey.dashscopeVoice) ?: "Cherry"
        set(value) {
            appCtx.putPrefString(PreferKey.dashscopeVoice, value)
        }

    //OpenAI 兼容 TTS
    var openaiTtsBaseUrl: String
        get() = appCtx.getPrefString(PreferKey.openaiTtsBaseUrl) ?: "https://api.openai.com"
        set(value) {
            appCtx.putPrefString(PreferKey.openaiTtsBaseUrl, value)
        }

    var openaiTtsApiKey: String
        get() = appCtx.getPrefString(PreferKey.openaiTtsApiKey) ?: ""
        set(value) {
            appCtx.putPrefString(PreferKey.openaiTtsApiKey, value)
        }

    var openaiTtsModel: String
        get() = appCtx.getPrefString(PreferKey.openaiTtsModel) ?: "tts-1"
        set(value) {
            appCtx.putPrefString(PreferKey.openaiTtsModel, value)
        }

    var openaiTtsVoice: String
        get() = appCtx.getPrefString(PreferKey.openaiTtsVoice) ?: "alloy"
        set(value) {
            appCtx.putPrefString(PreferKey.openaiTtsVoice, value)
        }
