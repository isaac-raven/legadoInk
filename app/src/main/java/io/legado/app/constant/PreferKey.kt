...
object PreferKey {
    ...
    const val isEInkUiStyle = "isEInkUiStyle"
    ...
    const val edgeTtsVoiceEn = "edgeTtsVoiceEn"

    //Edge TTS 朗读模式: 0中文音色读中英文 1中英文分别用各自音色 2英文音色读中英文
    const val edgeTtsReadMode = "edgeTtsReadMode"

    //通义千问 TTS (DashScope)
    const val dashscopeApiKey = "dashscopeApiKey"
    const val dashscopeModel = "dashscopeModel"
    const val dashscopeVoice = "dashscopeVoice"

    //OpenAI 兼容 TTS (VibeVoice自建等)
    const val openaiTtsBaseUrl = "openaiTtsBaseUrl"
    const val openaiTtsApiKey = "openaiTtsApiKey"
    const val openaiTtsModel = "openaiTtsModel"
    const val openaiTtsVoice = "openaiTtsVoice"
