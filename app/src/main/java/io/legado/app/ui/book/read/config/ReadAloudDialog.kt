...
class ReadAloudDialog : BaseDialogFragment(R.layout.dialog_read_aloud) {
    ...
    override fun onFragmentCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.run {
            ...
            ivVoice.setColorFilter(textColor)
    ...
    private fun initEvent() = binding.run {
        ...
        //音色切换(仅Edge TTS引擎显示)
        llVoice.isVisible = ReadAloud.ttsEngine == EdgeTts.ENGINE_ID
        llVoice.setOnClickListener { showVoiceSelect() }
    ...
    /**
     * Edge TTS 音色选择(中文声音/英文声音)
     * 切换后重建引擎配置并以当前进度重新朗读(不同音色的音频缓存按音色名自动隔离)
     */
    private fun showVoiceSelect() {
        context?.selector(
            getString(R.string.select_voice),
            listOf("中文声音", "英文声音(中英混读时朗读英文)")
        ) { _, type ->
            showVoiceList(type == 1)
        }
    }

    private fun showVoiceList(english: Boolean) {
        val voices = if (english) EdgeTts.EN_VOICES else EdgeTts.VOICES
        val current = if (english) AppConfig.edgeTtsVoiceEn else AppConfig.edgeTtsVoice
        val checkedIndex = voices.indexOfFirst { it.name == current }
        context?.selector(
            getString(R.string.select_voice),
            voices.map { it.display }
        ) { _, index ->
            if (index == checkedIndex) return@selector
            if (english) {
                AppConfig.edgeTtsVoiceEn = voices[index].name
            } else {
                AppConfig.edgeTtsVoice = voices[index].name
            }
            toastOnUi("已选择: ${voices[index].display}")
