...
class FontSelectDialog : BaseDialogFragment(R.layout.dialog_font_select),
    ...
    override fun onFragmentCreated(view: View, savedInstanceState: Bundle?) {
        ...
        if (fontPath.isNullOrEmpty()) {
            //未选择过字体目录时, 直接显示内置字体和SD卡fonts目录, 不再强制弹文件夹选择器
            loadDefaultFonts()
        } else {
    ...
    /**
     * 加载默认字体列表: SD卡fonts目录 + 内置字体目录
     */
    private fun loadDefaultFonts() {
        execute {
            val items = ArrayList<FileDoc>()
            //SD卡fonts目录(用户自定义字体的惯例位置)
            val sdFonts = File(FileUtils.getSdCardPath(), "Fonts")
            if (sdFonts.exists()) {
                items.addAll(sdFonts.listFileDocs {
                    it.name.matches(fontRegex)
                })
            }
            //应用内置字体目录(externalFiles/font)
